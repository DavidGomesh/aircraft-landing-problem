...
object PossibilitiesGenerator:
    ...
    def createOtherPossibilities(r: Runway, ur: Runway, a: Aircraft, rm: Seq[Aircraft], lr: Seq[Runway]): Option[Seq[Runway]] =
        createOtherPossibilities(r, ur, a).flatMap { r =>
            bruteForce(rm, Runway.update(lr, r))
        }

    // Auxiliary function to generate other possibilities in case of delays on a runway.
    def createOtherPossibilities(r: Runway, ur: Runway, a: Aircraft) = 
        (r.cost < ur.cost) match
            case true  => createBetterPossibility(r, a)
            case false => Some(ur)
    
    // Generate the best possibility of an aircraft on a runway
    def createBetterPossibility(r: Runway, a: Aircraft): Option[Runway] =
        createAllPossibilities(r, a).minByOption(_.cost)
