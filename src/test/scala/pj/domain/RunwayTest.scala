...
class RunwayTest extends AnyFunSuite:
    ...
    test("Adding an aircraft to Runway should increase its cost") {
        val aircraft = Aircraft(aicraftId1, Class2, target, maxTime, time)
    ...
    test("Setting a sequence of aircrafts to Runway should update its aircrafts") {
        val aircrafts = Seq(
        Aircraft(aicraftId1, Class1, target, maxTime),
        Aircraft(aicraftId2, Class1, target, maxTime))
        val runway = Runway(runwayId1, List(Class1, Class2, Class3))
    ...
    test("Runway is compatible with an aircraft") {
        val a = Aircraft(aicraftId1, Class4, target, maxTime, time)
    ...
    test("Checking compatibility between an aircraft and Runway should return a Result[Boolean]") {
        val aircraft = Aircraft("AC1", Class3, target, maxTime)
    ...
    test("Assigning a single aircraft to a Runway should return a Result[Runway]") {
        val aircraft = Aircraft(aicraftId1, Class1, target, maxTime, Some(0))
    ...
    test("Updating a Runway in a sequence of Runways should return the updated sequence") {
