...
import pj.domain.schedule.ScheduleMS03.generatePlan
...
object ScheduleProperties extends Properties("ScheduleProperties"):
    ...
    property("In a valid schedule, every aircraft was assigned a runway") = forAll(genAgenda) {
        (agenda) => {
            ...
            val scheduled = generatePlan(agenda)
                scheduled.fold(_ => false, (runway) => {
                    aircrafts.forall(air => runway.runways.flatMap(_.aircrafts.map(_.id)).contains(air.id))
                }
        )
    }
}
    ...
    property("Each aircraft should be scheduled for a runway which can handle it") = forAll(genAgenda) { 
        (agenda) =>{
            ...
            val scheduled = generatePlan(agenda)
            scheduled.fold(_ => false, (runway) => {
                aircrafts.forall(a => runway.runways.flatMap(_.aircrafts.map(aircraft => 
    ...
    property("An aircraft can never be scheduled before its target time") = forAll(genAgenda) { 
        (agenda) =>{
            ...
            val scheduled = generatePlan(agenda)
            scheduled.fold(_ => false, (runway) => {
                runway.runways.flatMap(_.aircrafts).foldLeft(true)((acc, aircraft) => acc && (aircraft.target <= aircraft.time.getOrElse(0)))
            })
        }
    }
