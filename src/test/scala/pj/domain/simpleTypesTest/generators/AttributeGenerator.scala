...
object AttributeGenerator extends Properties("GenAircraftTypes"):
    ...
    def genIdentifier(numCharacters: Int): Gen[Identifier] = Gen.listOfN(numCharacters, Gen.alphaNumChar).map(_.mkString).flatMap(str => 
      Identifier.apply(str).fold(_=> Gen.fail, Gen.const))

    def genClassType(minValue: Byte, maxValue: Byte): Gen[ClassType] = Gen.choose[Byte](minValue, maxValue).flatMap(n => 
      ClassType.apply(n).fold(_=> Gen.fail, Gen.const))

    def genAircraftTarget(minValue: Int, maxValue: Int): Gen[NonNegativeInteger] = Gen.choose(minValue, maxValue).flatMap(number =>
      NonNegativeInteger.apply(number).fold(_ => Gen.fail, Gen.const))

    def genAircraftTime(minValue: Int, maxValue: Int): Gen[NonNegativeInteger] = Gen.choose(minValue, maxValue).flatMap(number =>
      NonNegativeInteger.apply(number).fold(_ => Gen.fail, Gen.const))

    def genAircraftEmergency: Gen[Option[PositiveInteger]] = Gen.option(Gen.posNum[Int].flatMap { value =>
